...
def generate_designs_and_collect_results(
    ...
    parallelisms = config.get("PARALLELISM", [])
    if not parallelisms:
        raise ValueError("Config must contain a non-empty 'PARALLELISM' list.")

    results: list[dict[str, Any]] = []

    for model_idx, model in enumerate(model_list):
        model_base_name = model.name or f"model_{model_idx}"

        for parallelism in parallelisms:
            design_name = (
                f"{model_base_name}_P_{parallelism}_BW_{config.get('TOTAL_BITWIDTH', 'NA')}"
            )
            conf_dir = rtl_dir / design_name
            project_rtl_dir = conf_dir / "rtl"
            out_dir = conf_dir / "results"

            print(f"Generating RTL for {design_name} ...")

            try:
                lrg = build_lr_graph_from_model(model, parallelism=parallelism)

                if conf_dir.exists():
                    shutil.rmtree(conf_dir)

                project_rtl_dir.mkdir(parents=True, exist_ok=True)
                out_dir.mkdir(parents=True, exist_ok=True)

                lines_written = lr_graph_to_hardware(
                    lrg,
                    project_rtl_dir,
                    debug=False,
                )
                ...
                print(f"  LUT: {row.get('lut')}, FF: {row.get('ff')}, DSP: {row.get('dsp')}")
                print(f"  WNS(ns): {row.get('wns_ns')}, Fmax(MHz): {row.get('estimated_fmax_mhz')}")
