...
module fifo #(
    ...
    // Input stream: a "packet" of DATA_SIZE words
    input  logic [DATA_WIDTH-1:0] in_data  [0:DATA_SIZE-1],
    input  logic                  in_valid,
    output logic                  in_ready,

    // Output stream: a "packet" of DATA_SIZE words
    output logic [DATA_WIDTH-1:0] out_data [0:DATA_SIZE-1],
    ...
    typedef word_t                 slot_t [0:DATA_SIZE-1];

    localparam int PTR_W = (DEPTH <= 1) ? 1 : $clog2(DEPTH);

    // Storage: DEPTH slots, each slot has DATA_SIZE words
    slot_t ram [0:DEPTH-1];
    ...
    genvar g;
    generate
        for (g = 0; g < DATA_SIZE; g++) begin : GEN_OUT
            always_comb begin
                out_data[g] = ram[r_ptr][g];
            end
        end
    endgenerate
